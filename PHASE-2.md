...
| cursor | macOS pointer artwork since STC-239 (2026-09-02). Shape changes come from events-2 `cursor` events, which the helper does not emit yet, so a real take shows the arrow throughout. `project.cursor.style: "circle"` keeps the placeholder as an option |
