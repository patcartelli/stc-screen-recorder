...
| decision | status | evidence |
|---|---|---|
| Rule 2: the cursor is **never only in the video**. Telemetry is always captured, with no toggle | **holds; a take without telemetry does not start** | no code path or setting disables the tap, and since STC-315 a missing Input Monitoring grant (`tapCreate` returns nil) is an ERROR on `start` (`event-tap-unavailable`, `Capture.swift`'s `makeEventTap`/`begin`) rather than a warning over a video-only take. The refusal precedes `setupWriter()`, so no take directory is created at all. macOS raises its own Input Monitoring prompt on the first `tapCreate` (watched 2026-09-09), and that prompt says "keystrokes" for a tap this app keeps mouse-only — the app's refusal says so, because a generic system dialog is a fair reason to refuse a grant. **Still open, and it is a different question:** a tap DISABLED mid-take (`tapDisabledByTimeout`) is re-enabled and counted (`stats().tapDisabled`), so a starved run loop costs a gap in the track rather than the take. Nothing in the suite can starve a run loop on demand, which is why it was not settled here |
