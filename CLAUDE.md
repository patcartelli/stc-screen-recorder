...
| STC-342 | **interaction study #2, the selection overlay's handles — written 2026-09-10 on Linux, so EVERY JUDGEMENT ABOUT FEEL IS UNMADE.** Unlike the scrubber, STC-290 had already built and unit-tested (44 assertions) the drag/resize/move/nudge interaction, so this study's job was documentation plus an adversarial pass over math that was already believed correct, not a build. `app/src/selection.ts`'s header is now a 14-rule manifesto mirroring `scrubber.ts`'s, and rule 4 is the one that CHANGED: `resizeRect` used to let a resize shrink an EXISTING marquee below `MIN_SELECTION_POINTS`, and releasing there discarded the whole selection rather than holding it at a floor — a fresh drag ending too small correctly reads as "that was a click" (rule 3), but a resize that empties an established marquee with no warning is a different and worse thing, the same shape of defect as STC-338's rubber-band sign bug one study earlier. Fixed by flooring the signed resize extent LIVE and re-deriving the moving corner from the clamped value, so the corner opposite the dragged handle stays exactly fixed (mutation-tested: the new test fails `expected undefined to be defined` against the old code). Also split `handlePoint`/`handleAt` out of `overlay.ts` into a new pure module, `overlay-hittest.ts` — they touch only `Rect`/`Point`/`Handle`, never the DOM, but sat in a module that runs `document.*` at import time and so had never been unit tested; the new tests include a mutation-confirmed one for the handle tie-break that a floored (tiny) selection newly makes reachable. 3 new/changed assertions in `selection.test.ts` plus a new `overlay-hittest.test.ts` (5 assertions); full 3-pass typecheck and the real `still-overlay.e2e.test.ts` (Electron, under Xvfb) both green | a Mac: `docs/STC-342-RUNBOOK.md`. Whether the resize floor reads as a floor or as a stuck handle (and whether it wants the scrubber's rubber-band treatment instead of a hard clamp), and whether a floored selection's handles are still individually grabbable once they are closer together than `HANDLE_GRAB_PADDING` |
| STC-343 | **interaction study #3, the floating thumbnail's motion — written 2026-09-10 on Linux, so EVERY JUDGEMENT ABOUT FEEL IS UNMADE.** Same shape as STC-342: STC-296 had already built and tested (44+ unit assertions, a real-Electron E2E suite) the panel's appear/expand, swipe-to-discard, drag-out, right-click menu and stacking, across several passes, so this study's job was documentation plus an adversarial pass over motion math already believed correct. `app/src/thumbnail.ts`'s header is now a 10-rule manifesto mirroring `scrubber.ts`'s and `selection.ts`'s, and rule 6 is the one that CHANGED: a discard (swipe past threshold, or the right-click Delete) is an async round trip to trash the capture, and the panel's own timeout — a SEPARATE timer, in the main process, with no idea a discard is in flight — could fire in the gap before the delete resolves. `settleAndDestroy()` would hide the window, and if the delete then FAILED, the renderer's own recovery (restoring the panel, reporting the error) happened inside a window main had already hidden — invisible, and still headed for a silent destroy at `SETTLE_BACKSTOP_MS` regardless of the failure. The same shape of defect as the scrubber's rubber-band sign bug and the selection overlay's resize floor, one study each. Fixed with a `discarding` event sent as the FIRST thing `discard()` does, before `settling` is even set — `thumbnail-window.ts` clears the panel's timer on it, without marking the panel expanded. **Proven at the SOURCE, not live**: reproducing the actual race needs a real `BrowserWindow`, a real timer landing inside a real IPC round trip, AND an injected delete failure, all at once — `app/test/thumbnail-discard-race.test.ts` instead pins that the `discarding` event is sent before anything async and that both processes agree on its name and effect, with controls proving the assertions can fail. **A second, independent defect found in the same pass:** `thumbnail-preload.ts`'s own restated type for the panel→window event union (`"painted" \| "expanded" \| "done"`) had already silently drifted from the real one — missing `"redact"`, which the renderer's own `declare global` had — because nothing ever checks a preload's declared type against what a renderer actually sends across `contextBridge`. Widened to `unknown`, matching `overlay-preload.ts`'s `send`, which already carries no restated copy to drift. **Also written down as a rule rather than "fixed" because there is nothing to fix**: a successful drag-out does not end the panel's lifecycle (rule 8) — Electron's `startDrag` gives no completion signal at all, so the panel is left exactly as it was, still counting down; "nothing is lost by doing nothing" already covers what happens next regardless. Full 3-pass typecheck, 106 relevant unit assertions and 11 real-Electron E2E tests (under Xvfb) all green | a Mac: `docs/STC-343-RUNBOOK.md`. Whether a panel that sits untouched after a drag-out reads as correct or as the app forgetting what just happened, whether 26 px reads as a deck or noise, and whether 90 px/12 px are the right thresholds — none of it touched by this pass |
| STC-345 | **interaction study #4, the take library — the LAST in the series, written 2026-09-10 on Linux, so EVERY JUDGEMENT ABOUT FEEL IS UNMADE.** Same shape as STC-342/STC-343: STC-294 had already built and tested (58 relevant unit assertions, 12 real-Electron E2E tests) one scan, a grid, delete-with-no-orphans, so this study documents the decisions already made and does an adversarial pass over logic that shipped believing it correct. `app/src/library-items.ts`'s header is now a 10-rule manifesto mirroring the first three studies', and rule 9 is what the pass found: `still:duplicate` computed its destination from a filesystem snapshot and `mkdir(...,{recursive:true})`, which does not throw when the directory already exists — so two duplicates racing (a double-click, or two tiles duplicated inside the same second) could compute the SAME destination and both copy into it, interleaving two unrelated shots' files into one directory with nothing to say so, since the result still parses as some shot. The fourth instance of the exact defect shape this series has now found once per study (the scrubber's rubber-band sign bug, the selection overlay's resize floor, the thumbnail's discard race), and the same fix each time: claim the name in-process before the first `await`, release it in a `finally`. The logic moved out of `main.ts` into a new `duplicateTake` in `takes.ts` — plain node, no Electron — specifically so the race could be unit-tested directly rather than through an E2E; `app/test/takes.test.ts` proves two concurrent duplicates, of the same take AND of two different ones, never share a destination or interleave their files, mutation-tested (reverting to the pre-fix shape fails exactly the 3 concurrency tests and leaves the single-call test green). **A second, independent defect found in the same pass:** a byte-for-byte duplicate of `library.ts`'s `cameraSummary` — a function that reads a raw `anchors.json` blob — sat unreachable in `library-items.ts`, which is the pure/no-I/O module by this file's own stated rule. Nothing was wrong with the live behaviour (nothing ever called the dead copy), but it is the same "one value, two copies" defect this file keeps finding, just with the second copy having been dead all along; deleted, with a new structural test in `library-seam.test.ts` pinning `cameraSummary` to exactly one file. Full 3-pass typecheck, 58 relevant unit assertions and 12 real-Electron E2E tests (under Xvfb) all green | a Mac: `docs/STC-345-RUNBOOK.md`. Whether double-clicking Duplicate (or two tiles duplicated in the same second) now visibly produces two clean tiles — everything else about the library (the grid's look, the 500-take scroll) is unchanged from STC-294 and its own runbook still stands |
