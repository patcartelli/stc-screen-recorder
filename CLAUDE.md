...
| STC-309 | **increments 0-3 MERGED (#67) 2026-09-03; the spike is ANSWERED on hardware.** `NSCursor.currentSystem` sees other apps' pointers from the background helper: 599 samples, 0 nil, the I-beam matched `NSCursor.iBeam` byte-for-byte, 14 real flips in 20 s. It costs **1.04 ms mean / 41 ms max per sample**, so the `CursorSampler` (`helper/src/CursorShape.swift`) runs on its OWN thread, not the tap's; `orderedEvents` restores time order at write time. References are MEASURED at tap start from the four `NSCursor` built-ins. events.json is **v2**. Five unknown pointers (resize cursors) were correctly written as arrow | `pointingHand` CONFIRMED on a second probe over a link (2026-09-04) — all four shapes match. Next: `npm run test:capture`, a watched take, and `fixtures/real-session-cursor/` (`docs/STC-309-RUNBOOK.md` §2-3) |
| STC-306 | **helper half DONE 2026-09-04.** A display stream that dies under a live take (`didStopWithError` after `started`) now ends the take the way a display change does: `CaptureSession.onStreamDied` → `App.stop(reason: "stream-stopped")`, warning first, unsolicited `stopped` after, sidecars written and `display.mp4` finalised. `anchors-2` `stop.reason` gained `stream-stopped` / `stream-stopped-timeout`. The helper's FIRST production fault injector: `STC_CAPTURE_FAULT=stream-died` makes the session call its own delegate 0.5 s after a successful start, which is how `helper/test/stream-died.grant.test.ts` watches the path fire instead of reasoning about it. Written on Linux with no swiftc — CI's macOS runner is the first compile | `npm run test:capture` on the Mac: both new tests need the grant. NB the enum still lacks `quit` / `signal-N` / `stdin-closed`, which `shutdown` writes and nothing validates at load |
