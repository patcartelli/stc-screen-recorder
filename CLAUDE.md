...
| STC-309 | **increments 0-3 WRITTEN 2026-09-03, on Linux, UNCOMPILED** — the producer half of STC-239. `CursorSampler` (`helper/src/CursorShape.swift`) polls `NSCursor.currentSystem` at 30 Hz on the tap's own run loop, `decideCursorShape` (pure, `CaptureDecisions.swift`) emits only on change, `Capture.swift` appends `{kind:"cursor", shape}` on the session clock and writes events.json **v2**, time-ordered. References are MEASURED at tap start from the four `NSCursor` built-ins, not baked in. The spike is the helper's own `cursor-probe` command via `tools/test-host --cursor-probe`. `real-events`, `capture.grant` validate against `events-2`; `cursor-shape-names.test.ts` holds the Swift list to the schema | **`docs/STC-309-RUNBOOK.md`** — build on the Mac, run the probe (does `currentSystem` see other apps' pointers from a background process? that is still the open question), `npm run test:capture`, then a watched take: I-beam over a text field, hand over a link, arrow elsewhere |
