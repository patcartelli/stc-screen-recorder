...
| STC-371 | **asymmetric zoom shoulders — written 2026-09-14, on Linux, so `gate:identity`'s re-pin is NOT run.** Every preset used to be one critically-damped spring run both ways, so a zoom's push and its release were mirror images. `ZoomEasing` widened from `{omega, zeta}` to a pair (`in`/`out`), and `createZoomSim`'s `step` now picks the shoulder by which way the TARGET is pulling (`target === 1 ? easing.in : easing.out`) rather than by comparing state to target — the distinction the ticket itself calls out, because a future overshooting preset would flip mid-settle on the wrong one. Calm is unchanged and symmetric (ω 6 both ways); standard and snappy lean, each preset's shipped omega ×/÷ `Math.SQRT2` so the OVERALL speed is unchanged and only the push/release split moves. `TRANSFORM_VERSION` bumps to 4 and `TRANSFORM_HISTORY` says why — `ZOOM_PRESETS` was already declared to `transformFingerprint` (STC-325's own guard against exactly this), so the pinned fingerprint had to move the moment the numbers inside it did, same as `#108`/`#112` predicted. The crop is still the whole frame (STC-326 not landed), so no EXPORT pixel moves yet, but the editor's Zoom lane curve (STC-373, sampling `render()`'s own `zoom.amount`) already draws the new asymmetric shape. New test in `zoom.test.ts` measures push-to-half and release-to-half directly for each preset and asserts standard/snappy lean while calm does not, plus the existing "rests at 0" test updated for the new push-in omega it was silently asserting against. `npm run typecheck` clean on all three passes; the full non-grant/non-slow/non-e2e transform suite passes (39 files, 625 tests) | a Mac: `gate:identity` to re-pin the geometry, and a look at a real take through the two leaning presets — the numbers are reasoned, not seen, same as the ones they replace |
