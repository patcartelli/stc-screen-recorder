...
| `transform/src/zoom.ts` | auto-zoom stage 1 (STC-325) — `events → windows` and the easing spring. WHEN only; STC-326 decides where. Clicks and drags trigger; a plain move does not, and that one rule is the whole feature |
...
| `app/src/thumbnail.ts` | the post-capture floating thumbnail's pure decisions (STC-296) — showing/expanded state, the timeout floor, corner positioning math. No Electron, no DOM |
| `app/src/thumbnail-window.ts` | the panel's one real `BrowserWindow` and its one real timer — sizing, corner placement, hide-for-capture, settle-then-destroy. Mirrors `overlay-session.ts`'s split |
| `app/renderer/thumbnail.html`, `app/src/thumbnail-preload.ts`, `app/src/thumbnail-renderer.ts` | the panel's view: collapsed thumbnail, expanded mode picker + redact stub + copy/save. Reaches `still:export`/`still:frame` through its own, narrower bridge — the same main-process handlers every other exit uses |
...
| STC-296 | **post-capture floating thumbnail — CORE SLICE written 2026-09-08 on Linux, so the LOOK, THE ANIMATION AND THE CORNER PLACEMENT ARE UNSEEN.** A borderless always-on-top `BrowserWindow` (its own, separate from the main window and the overlay) appears after every still capture — button, hotkey, menu bar alike — positioned at a configurable corner of the display nearest the cursor, and auto-settles after a timeout (default 6 s, floor 3 s, both from the ticket). Click expands it in place into the whole still UI in v1: a decoration-mode preset picker, Copy, Save, and a Redact button (a disabled stub in this slice; STC-297 made it live, and the layout did not shift when it did — which was the point of stubbing rather than omitting it). Ignoring the panel (the timeout) OR closing it explicitly both settle it — export per the `settleAction` preference (save or copy), defaulting to save — so there is no path where a capture is silently lost; a `skip` preference bypasses the panel entirely and always copies. Timeout dismissal never blocks on the render: the window is hidden THE INSTANT the timeout fires (or a second capture replaces the panel) and the composite-and-export finishes in the background before the (already-invisible) window is destroyed. The OLD in-window still panel (STC-293) is deleted outright — this is the whole still UI now, not an alternative to it. **What is deliberately NOT in this slice** (see the ticket's own list): true `NSFilePromiseProvider` drag-out (STC-293 already chose write-then-hand-over-a-real-file for copy, for the same helper-already-idle reason, so drag-out follows `webContents.startDrag()` over a written cache file when it lands), a right-click menu, and multiple captures STACKING — a capture that arrives while one panel is showing REPLACES it, and the outgoing shot is still SETTLED (not merely discarded) rather than lost to the replacement, which the E2E proves. Capture-exclusion (`excludeWindowIds`, reusing STC-290's `windowIdOf`/`HIDE_SETTLE_MS`) is wired into every display-crop capture, including a full-display one, which previously sent none at all | a Mac: `docs/STC-296-RUNBOOK.md`. Everything about how it LOOKS and FEELS — the corner position, the appear/expand animation, whether `getMediaSourceId()` actually resolves a hidden panel's window id for real exclusion, and the drag-out/right-click/stacking follow-ups |
...
| STC-325 | **auto-zoom stage 1 — the WHEN signal, 2026-09-09.** `transform/src/zoom.ts`: the locked rule (300 ms lead, 2500 ms hold, merge at 2500) plus a critically damped spring on a 0..1 scalar, same 120 Hz step and checkpoint discipline as the cursor, seek-equals-step asserted across six checkpoint boundaries. **Triggers are clicks AND moves while a button is held** — `leftMouseDragged` is recorded as `kind: "move"`, so that one rule is what stops a motion-heavy take zooming continuously AND makes "a drag is one window" fall out of the locked rule instead of needing a special case. Both halves mutation-proven. Wired into `render()` as `FrameState.zoom` with the crop STUBBED full-frame, so it changes no pixels — by construction, not by measurement (see the trap). `TRANSFORM_VERSION` deliberately NOT bumped; `transform-version.ts` says why, where the omission would be made. **The ticket's keystroke premise was false** and is STC-327 | `gate:identity` with stage 1 on — it cannot run on Linux (no Chrome; the proxy blocks the install; bundled Chromium has no H.264), so CI is the first run. Presets are UNSEEN numbers, for the Music Network take to judge |
