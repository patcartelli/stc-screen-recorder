...
| `transform/src/zoom.ts` | auto-zoom stage 1 (STC-325) — the WHEN: windows from clicks (300 ms lead, 2500 ms hold, 2500 ms merge), the three easing presets, and a checkpointed 120 Hz spring. Hands out a SCALAR, never a rect: stage 2 owns where |
| `schema/project-4.schema.json` | project-3 plus `zoom`. `projectForWrite` emits the MINIMUM version that can express the document, so an unzoomed project stays v3 |
...
| STC-325 | **auto-zoom stage 1 — DONE 2026-09-09. The WHEN signal, and nothing about where.** `zoomWindows` is a pure `events → windows`: 300 ms lead, 2500 ms hold, windows closer than 2500 ms merge. A critically damped 120 Hz spring runs over them with the cursor's checkpoint discipline, and `render()` gained `zoom.amount` + `zoom.crop`. **Keystrokes are NOT in it, and that is the ticket's own correction** — `events-2` has three kinds and the tap's mask is mouse-only, so nothing here has ever recorded a keypress (STC-327). **A drag needs no special case**: both ends of a gesture are triggers, so a 5 s drag's two spans sit 2.2 s apart and the merge rule joins them — the ticket's "one window, not one per move" falls out rather than being coded. A ten-second drag SPLITS, deliberately, and that is pinned. **One departure, recorded**: the ticket asks for stiffness/damping PAIRS and these are single stiffnesses, because critical damping determines the second number and the only settings a free damping would buy are the overshoot a zoom cannot afford (a test asserts no overshoot on all three presets). Stage 1 hands out a SCALAR rather than a rect — that IS the stage-1/stage-2 seam, and a stage 1 producing rectangles would have had to invent the target the change-track amendment exists to take away from the cursor. `project-4` carries on/off, intensity and easing; the WINDOWS are never stored, so a re-take regenerates them. TRANSFORM_VERSION 3 | a Mac for nothing; **STC-313 for the only thing that matters** — neither committed fixture is demo-shaped, so the parent's hardest test ("Music Network should mostly decide NOT to zoom") is unverified, and the three preset numbers are reasoned from the cursor's 30 rad/s and have never been watched |
...
- **Two integrators that "do the same thing" must do it in the same ORDER, and a comment saying
  so is not evidence (STC-325).** `zoom.ts`'s spring mirrors `cursor.ts` — same semi-implicit
  Euler, same checkpoints, and its comment said as much. It stepped THEN incremented where
  cursor.ts increments THEN steps, so it stored state(tick+1) under checkpoint index tick and
  sampled the target a tick early. Seek and step then disagreed in the last digits of a value
  around 1e-63, which is invisible to any eye and fatal to the non-negotiable: preview and export
  walk different `t` sequences over one trajectory and must land bit-for-bit together. The
  determinism test across a checkpoint boundary is what caught it — the ONLY place the two paths
  execute different code, which is why that is where it is asserted.

- **A golden written before it is measured is a guess wearing a fixture's clothes (STC-325).**
  The real-take window counts were written as 6 and 3 from reading the code; the answer is 1 and 1,
  because `real-session` is 30 clicks roughly 300 ms apart and the merge rule correctly collapses
  the whole take into one window. Worse, the bounds were transcribed in round milliseconds and the
  real event times are not round: `10601` against `10601.397917`. Both are fixed the same way —
  the expectation is DERIVED from the fixture's own first and last trigger (`max(0, first − LEAD)`,
  `last + HOLD`), so it still fails if a constant or the merge rule moves but cannot be satisfied
  by copying a number out of a run. `fixtures/shot-v1` got this right by hand-deriving every
  number; this did not, until it failed.
  The finding underneath it is about the FIXTURES: neither committed take is demo-shaped —
  `real-session` was recorded to pin click semantics and is nearly continuous clicking — so
  "should mostly decide NOT to zoom" cannot be tested here at all, and the test file says so
  rather than letting a green run imply coverage.

- **A new optional field must be added where EVERY path returns, not only where you were looking
  (STC-325).** `parseProject` gained `project.zoom` in its main body, and `parseProject(null)`
  takes an early `return fallback` that never reached it — one function returning two shapes,
  which the trim tests caught by comparing the two against each other. The fix is in
  `defaultProject`, so the default IS the shape; patching the assertion would have left the
  inconsistency and removed the thing that noticed it.
